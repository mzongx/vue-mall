      ...
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li
                v-for="(item, index) in addressLimit"
                :key="index"
                :class="{'check': index === selectAddress}"
                @click="setAddress(index)"
              >
                <dl>
                  <dt>{{ item.addressName }}</dt>
          ...
          <div class="shipping-addr-more">
            <a class="addr-more-btn up-down-btn" href="javascript:;" :class="{'open': limit === addressList.length}" @click="more">
              more
              <i class="i-up-down">
        ...
        <div class="next-btn-wrap">
          <a class="btn btn--m btn--red" @click="nextOrder">Next</a>
        </div>
      </div>
    </div>
  </div>
...
import { getAddress, setdefault } from 'common/apis/users'
...
export default {
  ...
  computed: {
    addressLimit () {
      // 2.0移除了limit函数，所以在computed实现此功能
      return this.addressList.slice(0, this.limit)
    },
    addressId () {
      let address = this.addressLimit.filter((item) => {
        return item.isDefault
      })
      return address[0].addressId
    }
  },
  methods: {
    async _getAddress () {
    ...
    async _setdefault (addressId) {
      let params = {
        addressId
      }
      let addressRes = await setdefault(params)
      let addressData = addressRes.data
...
<style scoped lang="stylus">
  ...
  .lemall-msg-info span:before
    ...
    width: 16px;
    height: 16px;
    background: url(http://image3.lecloud.com/lemallus/20160921-91ac72bb-bcc8-4741-8f66-38c10a105043.png) no-repeat 0 0;
