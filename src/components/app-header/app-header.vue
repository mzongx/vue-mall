    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container">
        <div class="navbar-menu-container">
          <span class="navbar-link" v-show="nikeName">{{ nikeName }}</span>
          <a href="javascript:void(0)" class="navbar-link" @click="loginModalFlag = true" v-if="!nikeName">Login</a>
          <a href="javascript:void(0)" class="navbar-link" @click="_logOut" v-else>Logout</a>
            ...
            <div class="confirm-tips">
              ...
              <ul>
                <li class="regi_form_input">
                  ...
                  <input type="text" tabindex="1" name="nikeName" v-model="userName" class="regi_login_input regi_login_input_left" placeholder="User Name" data-type="nikeName">
                </li>
                <li class="regi_form_input noMargin">
                  <i class="icon IconPwd"></i>
                  <input type="password" tabindex="2"  name="password" v-model="userPwd" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Password" @keyup.enter="_login">
                </li>
              </ul>
            </div>
            <div class="login-wrap">
              <a href="javascript:;" class="btn-login" @click="_login">登  录</a>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async _logOut () {
      ...
      if (logoutRes.data.status === '0') {
        this.set_nikeName('')
        this.setCartList(true)
        this.set_cart(0)
      }
    },
    async _checkLogin () {
      let checkLoginRes = await checkLogin()
      let data = checkLoginRes.data
      if (data.status === '0') {
        this.set_nikeName(data.result)
        this._checkCart()
      } else {
    ...
    async _checkCart () {
      let cartRes = await cartcount()
      let resData = cartRes.data
      if (resData.status === '0') {
        this.set_cart(resData.result.cartCount)
      }
    }
  }
}
